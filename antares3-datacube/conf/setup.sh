...
/bin/bash -c "source /usr/local/bin/virtualenvwrapper.sh && alias python=python3 && mkvirtualenv antares && pip3 install dask distributed --upgrade && pip3 install bokeh && pip3 install --upgrade python-dateutil && pip install numpy==1.14.0 && pip install cloudpickle && pip install GDAL==$(gdal-config --version) --global-option=build_ext --global-option='-I/usr/include/gdal' && pip install rasterio==1.0.2 --no-binary rasterio && pip install scipy sklearn lightgbm fiona django --no-binary fiona && pip install --no-cache --no-binary :all: psycopg2 && pip install git+https://github.com/opendatacube/datacube-core.git@develop#egg=datacube[s3] && pip install boto3 botocore awscli --upgrade && pip install jupyter jupyterlab --upgrade && cd /home/madmex_user/git/antares3 && pip install -e ."
# HAndle conf files
